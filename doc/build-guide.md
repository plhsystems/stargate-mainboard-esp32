...
Source the ESP-IDF 6.1 environment in each terminal session:
```bash
...
```bash
alias get_idf='. ~/esp/esp-idf-6.1/export.sh'
```

### Step 2: Web Assets Generation

Web assets are automatically regenerated by CMake at build time — no manual step is needed. If you do want to run the tool manually:
...
The `--compress-all` flag gzip-compresses all text-based assets (HTML, CSS, JS, SVG, JSON, etc.), reducing firmware size by ~60–70 %.

**Generated files**:
- `WebServer/EmbeddedFiles.c` - C arrays with file contents (compressed where applicable)
...
**Build output**: `build/stargate-fw.bin` (~1.5 MB after gzip compression)
...
**Build output**: `build/pablo-board.bin` (~1.4 MB after gzip compression)
...
- [ ] Web assets regenerated (automatic at build time via CMake)
...
**Pinky Board**:
```bash
cd ~/stargate-mainboard-esp32/firmware/stargate-fw/pinky-board && . ~/esp/esp-idf-6.1/export.sh && idf.py build
```

**Pablo Board**:
```bash
cd ~/stargate-mainboard-esp32/firmware/stargate-fw/pablo-board && . ~/esp/esp-idf-6.1/export.sh && idf.py build
```

**Ring FW**:
```bash
cd ~/stargate-mainboard-esp32/firmware/ring-fw && . ~/esp/esp-idf-6.1/export.sh && idf.py build
```
...
cd ~/stargate-mainboard-esp32/firmware && \
  ...
  (cd stargate-fw/pablo-board && idf.py build) && \
