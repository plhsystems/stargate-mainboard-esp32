                            ...
                            struct ble_gatt_attr* attr, void* arg)
{
    if (0 != error->status)
    {
        ESP_LOGW(TAG, "Write response error: status=%d", error->status);
    }
    return 0;
}
...
SGResult RingBLEClient::WriteCharacteristic(const uint8_t* data, uint16_t len)
{
    ...
    const int rc = ble_gattc_write_flat(m_conn_handle, m_char_value_handle,
                                   data, len, onWriteComplete, nullptr);
    if (0 != rc)
    {
                        ...
                        void* arg)
{
    ...
    // chr == nullptr signals end-of-discovery; status may be BLE_HS_EDONE (not an error)
    if (nullptr == chr)
    {
        if (0 != error->status && BLE_HS_EDONE != error->status)
        {
    ...
    if (chr->properties & BLE_GATT_CHR_F_WRITE)
    {
        ...
        if (0 == ble_uuid_cmp(&chr->uuid.u, &CHAR_UUID.u))
        {
...
int RingBLEClient::gapEventHandler(struct ble_gap_event* event, void* arg)
{
    ...
    switch (event->type)
    {
        case BLE_GAP_EVENT_DISC:
        {
            ...
            if (0 == rc && nullptr != fields.name)
            {
                // Check if this is the RingFW-BLE device
                if (10 == fields.name_len && 0 == memcmp(fields.name, "RingFW-BLE", 10))
                {
                    ESP_LOGI(TAG, "Found Ring device!");

                    client->m_ring_found = true;
                    client->m_is_scanning = false;
        ...
        case BLE_GAP_EVENT_CONNECT:
        {
            ESP_LOGI(TAG, "Connection %s; status=%d",
                     0 == event->connect.status ? "established" : "failed",
            ...
            if (0 == event->connect.status)
            {
                ...
                // Discover only our specific characteristic by UUID (faster — one ATT round-trip)
                const int rc = ble_gattc_disc_chrs_by_uuid(event->connect.conn_handle,
                                                      1, 0xffff,
                                                      &CHAR_UUID.u,
...
void RingBLEClient::StartScan()
{
    ...
    // Use a finite scan window so it naturally ends and can be restarted
    const int rc = ble_gap_disc(BLE_OWN_ADDR_PUBLIC, pdMS_TO_TICKS(4000), &disc_params,
...
void RingBLEClient::ConnectToRing()
{
    ...
    conn_params.supervision_timeout = 0x0C80;  // 32s — generous timeout for WiFi/BLE coexistence
    ...
    const int rc = ble_gap_connect(BLE_OWN_ADDR_PUBLIC, &addr, 30000, &conn_params,
                             gapEventHandler, this);
    if (0 != rc)
    {
